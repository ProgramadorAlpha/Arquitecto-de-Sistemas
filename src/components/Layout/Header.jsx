...
const Header = ({ user, onLogout, toggleTheme, isDark, openManifesto, mantraSubtitle }) => {
  return (
    <header className="bg-slate-900 text-white p-6 pb-12 rounded-b-[40px] shadow-2xl relative overflow-hidden mb-8">
      {/* Top Gradient Bar */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-3 bg-white/5 px-4 py-2 rounded-xl border border-white/10">
              ...
              <div className="text-left">
                <p className="text-[10px] uppercase tracking-widest font-bold text-green-400">Online</p>
                <p className="font-bold text-white text-sm truncate max-w-[100px]">{user?.name || 'Arquitecto'}</p>
              </div>
            </div>
            ...
            >
              <ScrollText className="text-yellow-400 w-5 h-5 group-hover:scale-110 transition-transform" />
              <span className="text-white/50 text-[9px] font-bold uppercase tracking-wider">Manifiesto</span>
            </button>
